...
export const addTask = createAsyncThunk(
  ...
  async ({ project_id, section_id, taskObj }, { getState, dispatch }) => {
    console.log('TaskThunk => taskObj', project_id, section_id, "\n TaskObj \n", taskObj);
    try {
...
export const getTask = createAsyncThunk("task/getTask", async ({ project_id }, { getState }) => {
  try {
    const user = JSON.parse(localStorage.getItem('todoist_user'));
    if (!user) {
      throw new Error('No user found in local storage');
    ...
    for (const { section_id } of section.section) {
      ...
      const snapShot = await getDocs(taskCollectionRef, orderBy("task_add_date", "asc"))
